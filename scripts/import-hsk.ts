...
import type { Example } from '../src/shared/types.js';
...
async function generateExamples(
  ...
  const wordList = words.map(w => `${w.hanzi} (${w.pinyin}) [HSK ${w.hskLevel}]: ${w.english.join(', ')}`).join('\n');
  ...
  const stream = await anthropic.messages.stream({
    ...
    messages: [{
      role: 'user',
      content: `Generate ${numExamples} simple example sentence${numExamples > 1 ? 's' : ''} for each Chinese word below. Each sentence should:
...
async function importLevels(levels: number[], numExamples: number): Promise<void> {
  // Initialize database first
  await initDb();

  // Start rank after existing words
  let globalRank = getWordCount() + 1;
  ...
  for (const level of levels) {
    try {
      ...
      for (let i = 0; i < cleanedWords.length; i += batchSize) {
        const batch = cleanedWords.slice(i, i + batchSize);
        console.log(`  Generating examples for words ${i + 1}-${Math.min(i + batchSize, cleanedWords.length)}...`);
      ...
      console.log(`Imported HSK ${level}`);
    } catch (error) {
...
// Usage: npm run import-hsk [--examples=N] [levels]
// Examples:
...
function parseArgs(args: string[]): { levels: number[]; numExamples: number } {
  let numExamples = 1;
...
const { levels, numExamples } = parseArgs(process.argv.slice(2));
console.log(`Importing HSK levels: ${levels.join(', ')} with ${numExamples} example${numExamples > 1 ? 's' : ''} per word`);
importLevels(levels, numExamples).catch(console.error);
